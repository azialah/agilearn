    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden rounded-[2rem] p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <HardDrive className="size-5 text-[var(--color-accent-350)]" />
            <p className="mt-5 text-4xl font-semibold">{formatBytes(used)}</p>
            <p className="mt-1 text-sm text-[var(--color-ink-muted)]">
              of {formatBytes(limit)} available
            </p>
          </div>
          <span className="rounded-full bg-[var(--color-surface-2)] px-3 py-1 text-sm font-medium">
            {percent}% used
          </span>
        </div>
        <div className="mt-6 h-3 overflow-hidden rounded-full bg-[var(--color-surface-3)]">
          <div
            className={
              percent >= 95
                ? 'h-full bg-red-500'
                : percent >= 80
                  ? 'h-full bg-amber-500'
                  : 'h-full bg-gradient-to-r from-[var(--color-accent-350)] to-[var(--color-accent-500)]'
            }
            style={{ width: `${percent}%` }}
          />
        </div>
        {warning && (
          <p className="mt-4 flex gap-2 rounded-xl bg-[var(--color-surface-2)] p-3 text-sm text-[var(--color-ink-muted)]">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Card className="rounded-[2rem] p-6">
          ...
          <p className="mt-3 font-[var(--font-calligraphy)] text-xl text-[var(--color-accent-350)]">
